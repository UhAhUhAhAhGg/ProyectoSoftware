...
<<<<<<< HEAD
=======
    cancellation: e.cancellation ?? (e.cancellation_date || e.cancellation_reason || e.cancelled_by ? {
      date: e.cancellation_date || null,
      reason: e.cancellation_reason || null,
      internal_notes: e.cancellation_internal_notes || null,
      by: e.cancelled_by || null,
    } : null),
...
<<<<<<< HEAD
=======
  // Cancel event with metadata: try dedicated cancel endpoint, fallback to PATCH or DELETE
  cancelEvento: async (id, { reason = '', internal_notes = '', notify_buyers = true } = {}) => {
    // Try POST /events/{id}/cancel/
    let res;
    try {
      res = await apiFetch(`${EVENTS_URL}/api/v1/events/${id}/cancel/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reason, internal_notes, notify_buyers }),
      });
    ...
    // Fallback: PATCH event with status and cancellation fields
    try {
      const body = { status: 'cancelled', cancellation: { date: new Date().toISOString(), reason, internal_notes } };
      res = await apiFetch(`${EVENTS_URL}/api/v1/events/${id}/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
    ...
    // Last resort: call DELETE
    const del = await apiFetch(`${EVENTS_URL}/api/v1/events/${id}/`, { method: 'DELETE' });
    if (!del.ok) {
      const err = await del.json().catch(() => ({}));
      throw new Error(err.message || 'No se pudo cancelar/eliminar el evento.');
    }
    return { success: true };
  },

  // Fetch cancellation history (if backend supports it)
  getEventCancellations: async (id) => {
    try {
      const res = await apiFetch(`${EVENTS_URL}/api/v1/events/${id}/cancellations/`);
      if (!res.ok) return [];
      const data = await res.json();
