...
=======
  ...
  const confirmarEliminar = async () => {
    if (eventoAEliminar) {
      try {
        await eventosService.cancelEvento(eventoAEliminar.id, { reason: cancelReason || 'Cancelado por promotor via panel', internal_notes: '', notify_buyers: true });
      } catch (err) {
        alert(err.message || 'No se pudo cancelar el evento.');
      }
      setCancelReason('');
...
=======
                  onClick={async () => {
                    // Fetch full detail (including possible cancellation metadata)
                    const full = await eventosService.getEventoById(evento.id);
                    setEventoDetalle(full || evento);
                  }}
>>>>>>> 9507609 (Subiendo proyecto parte frontend Marcia)
...
=======
            <label style={{ display: 'block', marginTop: 8 }}>
              Motivo de cancelación (visible para compradores):
              <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} style={{ width: '100%', minHeight: 80, marginTop: 6 }} />
            </label>
>>>>>>> 9507609 (Subiendo proyecto parte frontend Marcia)
...
<<<<<<< HEAD
=======
  const cancellation = evento.cancellation || null;
>>>>>>> 9507609 (Subiendo proyecto parte frontend Marcia)
...
=======
          {cancellation && (
            <div style={{ marginTop: 12, background: '#fff0f0', padding: 12, borderRadius: 6 }}>
              <h4 style={{ margin: '0 0 8px 0' }}>Cancelación</h4>
              <div><strong>Fecha:</strong> {cancellation.date ? new Date(cancellation.date).toLocaleString('es-ES') : '—'}</div>
              <div><strong>Motivo:</strong> {cancellation.reason || '—'}</div>
              <div><strong>Responsable:</strong> {cancellation.by ? (cancellation.by.display_name || cancellation.by.username) : (evento.cancelled_by?.username || '—')}</div>
            </div>
          )}
>>>>>>> 9507609 (Subiendo proyecto parte frontend Marcia)
        </div>
